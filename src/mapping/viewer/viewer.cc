...
bool Viewer::InitDataPath(const YAML::Node& config_node) {
  ...
  map_path_ = data_path + "/data/slam_data/map";

  if (!FileManager::InitDirectory(map_path_, "Point Cloud Map"))
    return false;
...
bool Viewer::InitFilter(std::string filter_user,
  ...
  std::string filter_method = config_node[filter_user +
                              "_filter"].as<std::string>();
  LOG(INFO) << "viewer_" + filter_user << " select the method： "
            << filter_method;
  ...
  if (filter_method == "voxel_filter") {
    ...
    LOG(ERROR) << filter_method << " Not Found for " << filter_user;
    return false;
  }
...
bool Viewer::OptimizeKeyFrames() {
  ...
  while (all_index < all_key_frames_.size()) {
    all_key_frames_.at(all_index).pose = pose_to_optimize_ *
...
bool Viewer::SaveMap() {
  ...
  LOG(INFO) << "地图保存完成，地址是：" << std::endl << map_file_path
            << std::endl << std::endl;
